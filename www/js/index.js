...
function iniciaDB(tx){
	...
	tx.executeSql('CREATE TABLE IF NOT EXISTS Boleto (idabajo integer primary key AUTOINCREMENT,idBoleto integer UNIQUE, strQr text, strBarcode text, idCli interger, idCon integer,idLocB integer,nombreHISB text,documentoHISB text,strEstado text , hora_ingreso text , identComprador integer)');
	...
	tx.executeSql('CREATE TABLE IF NOT EXISTS empleado (id integer primary key AUTOINCREMENT, nombre text)');
